...
public class PrintModelRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<PrintModel>
{
	...
	public async Task<IRepositoryResponse<PrintModel>> GetByIdAsync(int id)
	{
		...
		try
		{
			var result = await appDbContext.PrintModels
										.Include(p => p.PrintOids)
										.Where(p => p.Id == id)
										.FirstOrDefaultAsync();
			if (result == null) return RepositoryResponse<PrintModel>.Failure(new List<string>(), $"Не удалось найти модель с id = {id}");
	...
	public async Task<IRepositoryResponse<PrintModel>> GetByModelNameAsync(string modelName)
	{
		...
		try
		{
			var result = await appDbContext.PrintModels
				...
				.FirstOrDefaultAsync();
			if (result == null) return RepositoryResponse<PrintModel>.Failure(new List<string>(), $"Не удалось найти модель с именем = {modelName}");
			return RepositoryResponse<PrintModel>.Success(result, "Модель найдена");
		}
		catch (Exception ex)
		{
	...
	public async Task<IRepositoryResponse<PrintModel>> UpdateAsync(PrintModel model)
	{
		...
		try
		{
			...
			modelExist.CalcTypeId = model.CalcTypeId;
