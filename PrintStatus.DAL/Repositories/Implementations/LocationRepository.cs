...
public class LocationRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<Location>
{
	...
	public async Task<IRepositoryResponse<Location>> GetByIdAsync(int id)
	{
		...
		try
		{
			...
			if (result == null) return RepositoryResponse<Location>.Failure(new List<string> { "" }, $"Не удалось найти местоположение с id = {id}");
	...
	public async Task<IRepositoryResponse<Location>> GetByTitleAsync(string title)
	{
		if (string.IsNullOrEmpty(title)) return new RepositoryResponse<Location>().HandleException(new ArgumentNullException(nameof(title)));
		try
		{
			var result = await appDbContext.Locations
								.Where(l => l.Name.Equals(title))
								.FirstOrDefaultAsync();
			if (result == null) return RepositoryResponse<Location>.Failure(new List<string> { "" }, $"Не удалось найти местоволожение с названием ");
